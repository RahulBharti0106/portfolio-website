...
function AdminSocial() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingLink) {
        ...
        await supabase.from('social_links').insert([{ ...formData, display_order: links.length + 1 }])
  ...
  const toggleVisibility = async (link) => {
    await supabase.from('social_links').update({ is_visible: !link.is_visible }).eq('id', link.id)
    fetchLinks()
  }

  const resetForm = () => {
    setFormData({ platform: '', url: '', icon: 'FiLink', is_visible: true })
    setEditingLink(null)
  }
          ...
          {links.map((link) => (
            <div key={link.id} className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                ...
                <button onClick={() => { setEditingLink(link); setFormData(link); setShowModal(true); }} className="btn-icon">
                  <FiEdit2 />
                </button>
            ...
            <div className="modal" onClick={(e) => e.stopPropagation()}>
              ...
              <form onSubmit={handleSubmit}>
                <div className="form-group">
                  ...
                  <input type="text" value={formData.platform} onChange={(e) => setFormData({ ...formData, platform: e.target.value })} required />
                </div>
                <div className="form-group">
                  <label>URL</label>
                ...
                <div className="form-group">
                  ...
                  <input type="text" value={formData.icon} onChange={(e) => setFormData({ ...formData, icon: e.target.value })} placeholder="FiGithub" />
                </div>
                <div className="form-group">
                  <label>
